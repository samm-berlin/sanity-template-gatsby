...
import { Maybe, SanityModuleFurtherReading, SanityPostOrStore } from '@/../types/graphql-types'
...
const FurtherReadingModule: FC<SanityModuleFurtherReading> = ({
  ...
  const moduleContent = (
    <Box px={6} mt={5} mb={5} mr="-0.9167rem" ml="-0.9167rem">
      <Box mt="2.25rem" mb="2.25rem" pl="0.9167rem" pr="0.9167rem" ml="auto" mr="auto">
        <StyledTypography component="h2" variant="subtitle1" maxWidth={12 / featuredPost.length}>
          {title && title}:
        </StyledTypography>
      </Box>
      <Grid alignContent="flex-start" container direction="row" spacing={0}>
        {featuredPost.map((postOrStore: Maybe<SanityPostOrStore>) => {
          return (
            <Post item xs={6} sm={6} md={12 / featuredPost.length} key={postOrStore?.id}>
              <Anchor href={`//${window?.location?.host || ''}/${postOrStore?.slug?.current}`}>
                <Image
                  style={{
                    backgroundImage: `url(${postOrStore?.meta?.ogImage?.asset?.url || ''})`
                  }}
                />
